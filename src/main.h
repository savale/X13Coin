...
#define FOUNDATION "xkMAHN5BA6Xtfevv7WGyLoVphzXtBJykXh"
...
public:
    ...
    {
        // X13Coin: the coin stake transaction is marked with the first output empty
    ...
    */
    ...
    bool GetCoinAge(CTxDB& txdb, uint64_t& nCoinAge) const;  // X13Coin: get transaction coin age
...
public:
    ...
    // X13Coin: block signature - signed by one of the coin base txout[N]'s owner
    ...
    // X13Coin: two types of block: proof-of-work or proof-of-stake
    ...
    // X13Coin: get max transaction timestamp
    ...
    bool AcceptBlock();
    bool GetCoinAge(uint64_t& nCoinAge) const; // X13Coin: calculate total coin age spent in block
...
public:
    ...
    unsigned int nFile;
    unsigned int nBlockPos;
    uint256 nChainTrust; // X13Coin: trust score of block chain
    int nHeight;

    int64_t nMint;
    int64_t nMoneySupply;

    unsigned int nFlags;  // X13Coin: block index flags
    enum  
    {
